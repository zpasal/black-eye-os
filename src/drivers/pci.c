...
uint32_t pci_read(uint32_t bus, uint32_t device, uint32_t function, uint32_t offset) {
    ...
    outpw(PCI_CONFIG, reg );

    return inpw(PCI_DATA);
}
...
uint8_t pci_find_class(int8_t class, uint8_t subclass, uint8_t* bus,  uint8_t* dev, uint8_t* function) {
  uint32_t b, d, f;
