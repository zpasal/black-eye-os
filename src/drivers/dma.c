...
#define channel_to_dma(CHAN) ((CHAN) <= 3 && (CHAN) > 0 ? (DMA_MASTER) : (DMA_SLAVE))
...
static inline void dma_enable_channel(struct dma_t *dma)
{
    ...
    outp(dma->mask_reg, dma->channel & 3);
}
...
static inline void dma_disable_channel(struct dma_t *dma)
{
    ...
    outp(dma->mask_reg, dma->channel | 4);
}
...
static int dma_set_page(struct dma_t *dma, addr_t addr)
{
    ...
    dma_clear_ff(dma);
    outp(dma->page_reg, (addr >> 16) & 0xFF);
    return 0;
}
...
static int dma_set_addr(struct dma_t *dma, addr_t addr)
{
    ...
    dma_clear_ff(dma);
    outp(dma->addr_reg, addr & 0xFF);
    outp(dma->addr_reg, (addr >> 8) & 0xFF);
...
static int dma_set_cnt(struct dma_t *dma, size_t cnt)
{
    ...
    dma_clear_ff(dma);
    outp(dma->cnt_reg, dma->buf_sz & 0xFF);
    outp(dma->cnt_reg, (dma->buf_sz >> 8) & 0xFF);
    return 1;
}
...
{
    dma_disable_channel(dma);
    outp(dma->mode_reg, DMA_MODE_SINGLE | DMA_MODE_AUTO_INIT |
